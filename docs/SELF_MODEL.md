...
The approved [Task 014 architecture](TASK_014_DESIGN.md) withholds autonomous psychological-semantic Candidate generation until a separate semantic grounding mechanism is approved. Current structural links and record counts cannot supply that meaning. A future transient system proposal must identify its proposer, actual mechanism, examined representations, counterevidence, limits, and unknowns. Unlike Task 013 user-authored draft wording, its generated claim, scope, explanation, and roles may themselves become invalid when a relied-upon source or context changes or is deleted. This creates no persistent Candidate, Pattern, dismissal, or semantic label.

The Owner-approved [Task 016 architecture](TASK_016_DESIGN.md) finds that completed Task 015 user comparisons support faithful summaries but do not directly authorize meaningful system Candidates: they do not identify the exact shared semantic aspect or classify material relative to a particular Candidate. Candidate System feasibility remains open. The preferred future v1 is a transient, deterministic user-grounded recurring-content Candidate mechanism: the user may explicitly supply a scoped shared aspect and claim-specific grounding, while NOUS would validate the complete selected scope, determine zero or one bounded proposals, and expose provenance. This is a design direction, not a new Self Model entity, persistent label, approved field, or implementation. A proposal remains distinct from accepted knowledge and a durable Pattern; user grounding is neither independent evidence nor user confirmation. A local semantic grounding model is only a separately reviewed future option.
