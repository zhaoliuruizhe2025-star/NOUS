...
The approved [Task 014 architecture](TASK_014_DESIGN.md) withholds autonomous psychological-semantic Candidate generation without semantic grounding. Structural links and record counts cannot supply that meaning. A transient system proposal must identify its proposer, actual mechanism, examined representations, limits, and unknowns. Unlike Task 013 user-authored draft wording, its generated claim, scope, explanation, and grounding may themselves become invalid when a relied-upon source or context changes or is deleted. This creates no persistent Candidate, Pattern, dismissal, or semantic label.

The completed [Task 016 architecture](TASK_016_DESIGN.md) found that Task 015 comparisons support faithful summaries but do not by themselves identify a shared semantic aspect or validate a particular Candidate. [Task 017](TASK_017_DESIGN.md) now implements one transient, deterministic user-grounded **Thought recurrence** Candidate mechanism: the user supplies the exact content X and claim-specific markings for every selected Thought; NOUS validates current sources, checks the Anchor-relative different-experience proof, and returns zero or one bounded system proposal with provenance and limitations. This does not introduce a durable Self Model entity, persistent label, accepted knowledge, or Pattern. User grounding is neither independent evidence nor user confirmation. Task 017 implementation passed Owner Code Review but awaits documentation review and final checkpoint/merge. A local semantic grounding model is only a separately reviewed future option.
