...
| Candidate | A reviewable, falsifiable possible recurring structure, proposed by NOUS or the user with proposer provenance | Candidate != confirmed user knowledge, even when well-supported; Task 013's user-authored and Task 017's first system-proposed slices are transient and user-initiated. |
...
The Owner-approved [Task 014 system-proposed Candidate architecture](TASK_014_DESIGN.md) finds that current Situation, Observation, and Thought structure cannot ground autonomous psychological-semantic Candidate generation. A stored relationship is not a semantic interpretation; tentative wording does not supply missing grounding. Any future system proposal requires an explicit user analysis trigger, a defined and inspectable mechanism, material counterevidence and unknown handling, and an ability to return no Candidate. The first eventual system-generated slice permits zero or one non-causal Candidate per analysis. At the Task 014 checkpoint, strictly transient user-assisted semantic comparison was selected for separate Task 015 scoping; Task 014 did not implement or authorize it.

The completed [Task 016 architecture](TASK_016_DESIGN.md) found that Task 015's Frame and Anchor-relative judgments support faithful comparison summaries, not direct psychological-semantic system Candidates: “similar” does not state the shared meaning, and “materially different” is not automatically counterevidence against a later claim. [Task 017](TASK_017_DESIGN.md) implements the first narrow, transient system-proposed Candidate: the user supplies one Thought content X and classifies every selected Thought against it; NOUS checks the complete current scope and derives only a bounded, zero-or-one cross-experience recurrence conclusion. The mechanism does not independently discover X, validate X as external truth, or create a Pattern. Its implementation passed Owner Code Review; Task 017 awaits documentation review and final checkpoint/merge. After merge, a dedicated Candidate Product Test must examine correctness, cognitive contribution, interaction cost, false authority, mixed evidence, and zero-output quality before Candidate semantics expand. A local semantic grounding model remains a separately reviewed future option; external AI APIs are not required for normal use.
