...
The Owner-approved [Task 016 architecture](TASK_016_DESIGN.md) reassesses the now-complete Task 015 comparison foundation. Its Frame and Anchor-relative user judgments support faithful comparison summaries, not direct psychological-semantic system Candidates: “similar” does not state the shared meaning, and “materially different” is not automatically counterevidence against a later claim. The preferred future direction is one complete, deterministic User-Grounded Candidate System v1 mechanism. A human may explicitly ground a shared aspect and its relation to relevant records; NOUS must still perform bounded, Candidate-specific validation over the selected scope before returning zero or one transient proposal. Exact v1 rules and implementation require separate approval. After implementation, a dedicated Candidate Product Test must assess correctness, system cognitive contribution, and interaction cost. A local semantic grounding model is only a separately reviewed future option; external AI APIs are not required for normal use.
